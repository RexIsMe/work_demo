...
import com.thalys.dc.scbi.sparkstreamingimpl.entity.CaseDCRecordEntity
...
//    ssc.checkpoint("hdfs://hadoop-master:9000/spark/checkpoint/ck1")
    ...
    val recordDS: DStream[CaseDCRecordEntity] = dStream.map(x => {
      JacksonUtils.readValue(x, classOf[CaseDCRecordEntity])
    })
...
//    val ssc: StreamingContext = StreamingContext.getActiveOrCreate("hdfs://hadoop-master:9000/spark/checkpoint/ck1", () => createSSC())
    val ssc: StreamingContext = StreamingContext.getActiveOrCreate(() => createSSC())
    ssc.start()
