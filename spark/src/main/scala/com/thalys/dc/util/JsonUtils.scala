...
object JsonUtils {
  ...
  def toObject[T](content: String, classType: Class[T]): T = {
    val t: T = mapper.readValue(content, classType)
    t
  }
  ...
  def test3 ={
    val rootNode:JsonNode = mapper.createObjectNode();

    val childNode1:JsonNode = mapper.createObjectNode();
    (childNode1.asInstanceOf[ObjectNode]).put("name1", "val1");
    (childNode1.asInstanceOf[ObjectNode]).put("name2", "val2");

    rootNode.asInstanceOf[ObjectNode].set("obj1", childNode1);
