...
//          stream.close();
//        } catch (IOException e) {
//          logger.error("close fileStream 111" + parentDir.toPath(), e);
//        }
//      }
//    }
//    return result;
...
//    List<File> result = Lists.newArrayList();
//    // 使用非递归的方式遍历文件夹
//    // 限制 子公司名 和 ip 的组合个数上限为50个
//    Queue<File> dirs = new ArrayBlockingQueue<>(50);
...
//            stream.close();
//          } catch (IOException e) {
//            logger.error("close fileStream 222" + parentDir.toPath(), e);
//          }
//        }
//      }
...
//   * 根据文件夹全路径判断是否符合监听条件
//   * 只监听当天文件夹下的文件
//   * eg ：/opt/dcDir/common/thalys.erp/null/2020/11/12
//   * @param f
//   * @return
//   */
...
//    //这里获取的月份是从0开始的
//    Integer monthNum = cal.get(Calendar.MONTH) + 1;
//    Integer dateNum = cal.get(Calendar.DATE);
//    boolean result = false;
//    String[] split = f.getAbsolutePath().split("/");
//    if(split.length < 7){
//      result = true;
//    }else if(split.length == 7 && yearNum.equals(Integer.valueOf(split[6]))){
//      result = true;
//    } else if (split.length == 8 && yearNum.equals(Integer.valueOf(split[6])) && monthNum.equals(Integer.valueOf(split[7]))){
//      result = true;
//    } else if (split.length == 9 && yearNum.equals(Integer.valueOf(split[6])) && monthNum.equals(Integer.valueOf(split[7])) && dateNum.equals(Integer.valueOf(split[8]))){
//      result = true;
//    }
//    return result;
//  }
